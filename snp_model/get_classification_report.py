...
def get_patient_precision_recall(hp,feature):
    ...
    base_dir ='C:/Users/hadar/Downloads/biomedical_eng/winter_2022/research/base_snp/'
    with (open(f"{base_dir}f_scores.p", "rb")) as openfile:
        data = pickle.load(openfile)
        f_base = data['f']
        data['f_BP'] = f_score_list
    pickle.dump( data, open( f"{hp['root_dir']}f_scores.p", "wb" ) )
...
def get_mean_pr(root):    
    ...
    labels_lo = np.array(total_labels[index_lo][0])
    ...
    labels_hi = np.array(total_labels[index_hi][0])

    total_labels = np.concatenate(total_labels)
    total_probs = np.concatenate(total_probs)   
                                    
    return total_labels,total_probs, probs_hi,probs_lo, labels_lo,labels_hi,total_ap
...
def plot_base_sub(hp,feature):
    ...
    root2 = f"{hp['root_dir']}"
    total_labels1,total_probs1, probs_hi1,probs_lo1, labels_lo1,labels_hi1,aps1 = get_mean_pr(root1)
    precision1, recall1, _ = precision_recall_curve(total_labels1, total_probs1) 
    precision_hi1, recall_hi1, _ = precision_recall_curve(labels_hi1, probs_hi1)
    precision_lo1, recall_lo1, _ = precision_recall_curve(labels_lo1, probs_lo1)   
    total_labels2,total_probs2, probs_hi2,probs_lo2, labels_lo2,labels_hi2, aps2 = get_mean_pr(root2)
    precision2, recall2, _ = precision_recall_curve(total_labels2, total_probs2)  
    precision_hi2, recall_hi2, _ = precision_recall_curve(labels_hi2, probs_hi2)
    precision_lo2, recall_lo2, _ = precision_recall_curve(labels_lo2, probs_lo2)  
    ...
    bottom_bar = mpatches.Patch(color='blue', label='$BP-CNN_{SNP}$ (area='+ap2+r'$\pm$'+std2+')')
